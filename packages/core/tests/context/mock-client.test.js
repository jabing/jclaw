...
describe('MockClient', () => {
    ...
    describe('addResource', () => {
        it('should throw when not connected', async () => {
            await expect(client.addResource('/path')).rejects.toThrow('not connected');
        });
        ...
        it('should store resource with timestamp', async () => {
            ...
            expect(resource?.addedAt.getTime()).toBeGreaterThanOrEqual(beforeAdd.getTime());
            expect(resource?.addedAt.getTime()).toBeLessThanOrEqual(afterAdd.getTime());
        });
...
//# sourceMappingURL=mock-client.test.js.map