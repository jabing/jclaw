...
describe('OpenVikingClient', () => {
  ...
  describe('connect', () => {
    ...
    it('should throw when server returns non-OK status', async () => {
      ...
      await expect(client.connect()).rejects.toThrow(
        'Failed to connect to OpenViking: Server returned 500'
      );
      expect(client.isConnected()).toBe(false);
    });
    ...
    it('should throw when server is not available', async () => {
      ...
      await expect(client.connect()).rejects.toThrow(
        'Failed to connect to OpenViking: Connection refused'
      );
      expect(client.isConnected()).toBe(false);
    });
    ...
    it('should handle timeout errors', async () => {
      ...
      await expect(client.connect()).rejects.toThrow(
        'Failed to connect to OpenViking'
      );
    });
  });
  ...
  describe('query', () => {
    it('should throw when not connected', async () => {
      await expect(client.query('test question')).rejects.toThrow(
        'Client not connected'
      );
    });

    it('should query successfully when connected', async () => {
      mockFetch.mockResolvedValueOnce({ ok: true } as Response); // connect
      mockFetch.mockResolvedValueOnce({
        ok: true,
  ...
  describe('addResource', () => {
    it('should throw when not connected', async () => {
      await expect(client.addResource('/path/to/resource')).rejects.toThrow(
        'Client not connected'
      );
    });
    ...
    it('should handle network errors during addResource', async () => {
      ...
      await client.connect();
      await expect(client.addResource('/path')).rejects.toThrow(
        'Add resource failed: Connection lost'
      );
    });
  });
