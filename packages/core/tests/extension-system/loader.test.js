...
describe('ExtensionLoader', () => {
    ...
    const createMockExtension = (name, options = {}) => ({
        name,
    ...
    describe('unload', () => {
        ...
        it('should fail when other extensions depend on it', async () => {
            ...
            await loader.load(createMockExtension('dependent-ext', { dependencies: ['base-ext'] }));
    ...
    describe('loadAll', () => {
        ...
        it('should load extensions in dependency order', async () => {
            const loadOrder = [];
            const createTrackedExtension = (name, deps) => createMockExtension(name, {
                dependencies: deps,
                install: async () => {
                    loadOrder.push(name);
                },
    ...
    describe('validateExtension', () => {
        ...
        it('should reject null/undefined', () => {
            expect(loader.validateExtension(null)).toBe('Extension must be an object');
            expect(loader.validateExtension(undefined)).toBe('Extension must be an object');
        });
        it('should reject missing name', () => {
            ...
            expect(loader.validateExtension(extension)).toBe('Extension must have a string "name" property');
        });
        it('should reject missing version', () => {
            const extension = { name: 'test', description: 'test', capabilities: [] };
            expect(loader.validateExtension(extension)).toBe('Extension must have a string "version" property');
        });
        it('should reject missing description', () => {
            const extension = { name: 'test', version: '1.0.0', capabilities: [] };
            expect(loader.validateExtension(extension)).toBe('Extension must have a string "description" property');
        });
        it('should reject missing capabilities', () => {
            const extension = { name: 'test', version: '1.0.0', description: 'test' };
            expect(loader.validateExtension(extension)).toBe('Extension must have an array "capabilities" property');
        });
        it('should reject invalid capability', () => {
            ...
            expect(loader.validateExtension(extension)).toBe('Capability at index 0 must have a string "name" property');
        });
        it('should reject invalid dependencies type', () => {
            ...
            expect(loader.validateExtension(extension)).toBe('Extension "dependencies" must be an array of strings');
        });
    });
});
