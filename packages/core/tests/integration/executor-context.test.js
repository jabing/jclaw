...
import { ExtensionRegistry, ExtensionLoader, CapabilityRouter, } from '../../src/extension-system/index.js';
describe('Integration: Executor + Context + Extension', () => {
    describe('Executor with MockClient', () => {
        ...
        it('should allow querying context with custom responses', async () => {
            ...
            await context.connect();
            // Set custom response
            context.setQueryResponse('What is JClaw?', 'JClaw is a self-evolving agent framework');
            // Query context
    ...
    describe('Full Workflow', () => {
        it('should execute command, store in context, via extension', async () => {
            ...
            expect(router.validateInput('execute-and-store', invalidInput)).toBe(false);
            await context.disconnect();
        });
    ...
    describe('Error Handling', () => {
        it('should handle disconnected context operations gracefully', async () => {
            ...
            await expect(context.query('test')).rejects.toThrow('Client not connected');
            await expect(context.addResource('/test.txt')).rejects.toThrow('Client not connected');
        });
        it('should handle capability conflicts between extensions', async () => {
            ...
            expect(() => registry.register(ext2)).toThrow('Capability "shared-capability" is already registered');
        });
...
//# sourceMappingURL=executor-context.test.js.map